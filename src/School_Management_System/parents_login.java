...
public class parents_login implements ActionListener {
    JFrame frame;
    JTextField username;
    ...
    parents_login(){
        ...
        frame = new JFrame("Parents Login");
        frame.setBounds(300, 200, 500, 500);
        frame.getContentPane().setBackground(Color.WHITE);
        frame.setLayout(null);

        ImageIcon i1 = new ImageIcon(ClassLoader.getSystemResource("img/parent_login.jpg"));
        ...
        JLabel title = new JLabel("Parents Login", JLabel.CENTER);
...
//        System.out.println(c.getSelectedItem());
    ...
    public void actionPerformed(ActionEvent ae){
        if (ae.getSource() == login){
            ...
            try {
                Conn con = new Conn();
                String q = "select * from parents where Child_ID='"+id+"' and password='"+password_+"'";
                System.out.println(q);
                ResultSet rs = con.stmt.executeQuery(q);
                if (rs.next()){
                    new parents_front(id, class_);
