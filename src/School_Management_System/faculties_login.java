...
public class faculties_login implements ActionListener {
    JFrame frame;
    JTextField username;
    ...
    faculties_login(){
        ...
        frame = new JFrame("Faculties Login");
        frame.setBounds(300, 200, 500, 500);
        frame.getContentPane().setBackground(Color.WHITE);
        frame.setLayout(null);

        ImageIcon i1 = new ImageIcon(ClassLoader.getSystemResource("img/faculty_login.jpg"));
        ...
        c.add("Principal");
        c.add("Accountant");
        c.add("Data Entry Operator");
        c.setBounds(230, 260, 200, 30);
        c.setFont(new Font("Franklin Gothic", Font.PLAIN, 25));
        img.add(c);
        ...
        login.setFont(font3);
        login.setBounds(120, 320, 100, 40);
    ...
    public void actionPerformed(ActionEvent ae){
        if (ae.getSource() == login){
            ...
            String post_ = c.getSelectedItem();

            try {
                Conn con = new Conn();
                String q = "select * from faculty where id='"+id+"' and password='"+password_+"' and post = '"+post_+"'";
//                System.out.println(q);
                ResultSet rs = con.stmt.executeQuery(q);
                if (rs.next()){
                    name = rs.getString("Name").toUpperCase();
                    if (post_.equalsIgnoreCase("Principal")){
                        new principal(id, name);
                        frame.setVisible(false);
                    }else if (post_.equalsIgnoreCase("Accountant")){
                        new accountant(id, name);

                    }else if (post_.equalsIgnoreCase("Data Entry Operator")){
                        new dataEntry(id, name);
